    ...
    class asio_socket::asio_socket_impl
    {
        ...
        public:
            size_t send(const char *buf, size_t size)
            {
                ...
                throw fflerror("Failed in asio::write(%p, %zu): %s", buf, size, ec.message().c_str());
            }
            ...
            size_t recv(char *buf, size_t size)
            {
                ...
                if(auto size_recv = asio::read(m_socket, asio::buffer(buf, size), [this](std::error_code &, size_t done) -> size_t
                {
                    ...
                    if(ec_ctrl){
                        throw fflerror("Failed in socket::io_control(byte_readable) : %s", ec_ctrl.message().c_str());
                    }
                ...
                throw fflerror("Failed in asio::read(%p, %zu): %s", buf, size, ec.message().c_str());
            }
    };
}
